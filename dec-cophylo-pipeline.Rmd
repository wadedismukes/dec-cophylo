...
br <- 0.06
dr <- 0.0
trr <- as.vector(c(0.0, 0.035, 0.1, 0.5))
gbr <- 0.0
gdr <- 0.0
reps <- 50
nl <- 1
...
run_sim_regime <- function(sim_dir, prefix_fn, num_reps){
    ...
    for (i in 0:(num_reps-1)){
        ...
        pruned_symb_tree <- drop.tip(symb_nwck_tr,grep('X', symb_nwck_tr$tip.label, value = TRUE))
...
print_rev_script <- function(dir_name, prefix_fn, reps){
    ...
    for(i in 0:(reps-1)){
        rev_out_fn <- paste0(rev_script_dir,
                             str_sub(prefix_fn, end = -2L),
                                     "/run_epoch_",
                                     prefix_fn,
                                     i,
                                     ".Rev")
        system2("mkdir",
                c("-p", paste0(rev_script_dir, str_sub(prefix_fn, end = -2L))))
        ...
        cat("moves.append( mvScale(dispersal_rate, weight=5) )\n")
        ...
        cat("monitors.append( mnScreen(printgen=100, dispersal_rate, extirpation_rate) )\n")
...
```{r print-data-files-and-scripts}
dir_name <- c("data/0_hsrate/", "data/0.1_hsrate/", "data/0.035_hsrate/", "data/0.5_hsrate/")
prefix_fn <- c("hsrate_0_", "hsrate_0.1_", "hsrate_0.035_", "hsrate_0.5_")
